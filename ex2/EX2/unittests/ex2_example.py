...
def ex2(inp_dir: str, out_dir: str, logfile: str):
    ...
    filenames = glob.glob(os.path.join(inp_dir, '**', '*'), recursive=True)
    assert isinstance(filenames)
    filenames.sort()
    ...
    for filename in filenames:
        ...
        if not (filename.endswith('.jpg') or filename.endswith('.JPG') or filename.endswith('.JPEG')
            ...
            with open(logfile, 'a') as lf:
                print(f"{filename[len(inp_dir)+1:]};1", file=lf)
            # Skip to next file if rule was violated
            continue
        # Check for rule 2
        if not os.path.getsize(filename) > 1e4:
            with open(logfile, 'a') as lf:
                print(f"{filename[len(inp_dir)+1:]};2", file=lf)
            continue
        # Check for rule 3
        try:
            ...
            with open(logfile, 'a') as lf:
                print(f"{filename[len(inp_dir)+1:]};3", file=lf)
            continue
        # Check for rule 4
        ...
        if not (len(image_shape) == 2 and min(image_shape) >= 100):
            with open(logfile, 'a') as lf:
                print(f"{filename[len(inp_dir)+1:]};4", file=lf)
            continue
        # Check for rule 5
        if np.std(np_image) <= 0:
            with open(logfile, 'a') as lf:
                print(f"{filename[len(inp_dir) + 1:]};5", file=lf)
            continue
        # Check for rule 6
        ...
        if np_image_hash in hash_list:
            with open(logfile, 'a') as lf:
                print(f"{filename[len(inp_dir)+1:]};6", file=lf)
            continue
        ...
        valid_files += 1
        shutil.copy(filename, os.path.join(out_dir, f"{valid_files:06d}.jpg"))
